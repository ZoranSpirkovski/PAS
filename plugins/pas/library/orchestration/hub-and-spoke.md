...
6. **When feedback is enabled**: carry `${CLAUDE_PLUGIN_ROOT}/library/self-evaluation/SKILL.md` for shutdown
...
- Self-evaluation instructions: "Before returning your final result, if feedback is enabled (see `${CLAUDE_PLUGIN_ROOT}/pas-config.yaml` or consumer `.pas/config.yaml` override), read `${CLAUDE_PLUGIN_ROOT}/library/self-evaluation/SKILL.md` and write feedback to `.pas/workspace/{process}/{slug}/feedback/{your-name}.md`"
...
5. **Self-evaluation instructions** (when feedback enabled): "Before returning your final result, read `${CLAUDE_PLUGIN_ROOT}/library/self-evaluation/SKILL.md` and write feedback to `.pas/workspace/{process}/{slug}/feedback/{your-name}.md`"
...
4. Classify the user's response using `${CLAUDE_PLUGIN_ROOT}/library/message-routing/SKILL.md`:
