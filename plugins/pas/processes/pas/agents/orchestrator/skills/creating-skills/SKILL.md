...
- Check existing skills within the owning agent (`<marketplace>/plugins/<plugin>/processes/{process}/agents/{agent}/skills/`)
- Check existing skills within the process (`<marketplace>/plugins/<plugin>/processes/{process}/`)
- Check `${CLAUDE_PLUGIN_ROOT}/library/` for global skills that already do this
