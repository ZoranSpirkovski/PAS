...
- **Phase Advancement Test (cycle-17):** Before binding, ask: *is this session about to advance a phase in this workspace?* If no, do not bind. "I see an in-progress workspace and I should register myself somewhere" is not a reason to bind. A fresh session running unrelated work — user invoked a different skill, asked an off-topic question, opened the project to read code — MUST leave the workspace's `status.yaml` untouched: no `current_session:` write, no `sessions:` append, no "off-topic" note added at stop time. The workspace belongs to the session that is actively advancing it. This is the orchestrator-side counterpart to the hook-side auto-bind suppression — the doctrine is the same, applied at every layer that can write the binding.

**Origin (cycle-17 extension):** Recurrence in consumer transcript `pas-misrouted-and-migration-ts` (Tangled-Roots-V1). Cycle-16 closed the hook-side auto-bind path, but the offending status.yaml accumulated 8 `sessions:` entries — 5 with "off-topic — user pivoted to ..." notes — because the orchestrator itself, plus a consumer-side skill, kept registering fresh sessions in the existing in-progress workspace and writing exit notes at stop time. The cycle-16 fix held in code; the surrounding documentation kept telling readers to bind.
